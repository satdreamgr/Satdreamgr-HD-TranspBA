...
class TranspBaServiceInfo(Converter, object):
    ...
    def getText(self):
        ...
        if self.type == self.SERVICENAME:
            ...
            if config.plugins.TranspBaServiceInfoServiceInfo.showServiceNumber.value == True and number != '':
                text = '%s. %s' % (number, name)
            else:
                text = name
            if config.plugins.TranspBaServiceInfo.showOrbitalPosition.value == True and orbital != '':
                if config.plugins.TranspBaServiceInfo.orbitalPositionType.value == 'name':
